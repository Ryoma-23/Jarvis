...
atmosphere plane, three planar volumetric Glow layers, and one immutable Core
particle field. The background intentionally contains no point field or stars.
EffectComposer renders the scene through RenderPass and UnrealBloomPass;
...
audio resonance, atmospheric drift, and transient impulses.
...
Geometry, Material, Glow planes, background plane, and WebGLRenderer. Hidden
documents skip rendering. Adaptive quality changes DPR without rebuilding scene
objects; compact widths reduce decorative CSS intensity without reallocating.
